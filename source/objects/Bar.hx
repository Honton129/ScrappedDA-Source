...
class Bar extends FlxSpriteGroup
{
	...
	public function new(x:Float, y:Float, image:String = 'healthBar', valueFunction:Void->Float = null, boundX:Float = 0, boundY:Float = 1)
	{
		...
		if (image == 'healthBar') //i hate flixel
		{
			bg = new FlxSprite(0, -17).loadGraphic(Paths.image('UI/' + image));
			bg.antialiasing = ClientPrefs.data.antialiasing;
			barWidth = Std.int(bg.width - 25);
			barHeight = Std.int(bg.height - 36);
		}
		else
		{
	...
	public function updateBar()
	{
		...
		if (bg.graphic == Paths.image('UI/healthBar')) //I REALLY HATE FLIXEL
		{
			leftBar.setPosition(bg.x + 10, bg.y + 15);
			rightBar.setPosition(bg.x + 10, bg.y + 15);	
		}
		else
		{

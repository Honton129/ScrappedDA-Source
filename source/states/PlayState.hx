...
class PlayState extends MusicBeatState
{
	...
	// SHADERS
	var vcrShader:VCRShader;
	var chromShader:ChromaticAbberation;
	var vcrFilter:ShaderFilter;
	...
	public static var curSong:String = ""; //changed it cuz it doesn't lets be used on Pause Menu
	...
	public var healthBarVisible:Bool = true;
	...
	public var overlayMuxi:BGSprite;
	...
	override public function create()
	{
		...
		if (SONG == null) //The song can't be found so we load "Test" instead for prevent crashes
		...
		switch (curStage)
		{
			case 'philly': new states.stages.Philly(); //Pico Stage (Day 1)
			case 'philly-2': new states.stages.PhillyD2(); //Pico Stage (Day 2)
			case 'philly-3': new states.stages.PhillyD3(); //Pico Stage (Day 3)

			case 'spooky': new states.stages.Spooky(); //Spooky Stage (Day 1)
			case 'spooky-2': new states.stages.SpookyD2(); //Spooky Stage (Day 2)
			case 'spooky-3': new states.stages.SpookyD3(); //Spooky Stage (Day 3)

			//SENPAI

			case 'highway': new states.stages.Highway(); //Mom Stage (Day 1)
			case 'highway-2': new states.stages.Highway(); //Mom Stage (Day 2)
			case 'highway-3': new states.stages.Highway(); //Mom Stage (Day 3)

			case 'stage': new states.stages.StageWeek1(); //Default Stage
		}
		...
		overlayMuxi = new BGSprite('UI/overlay');
		overlayMuxi.alpha = 0.15;
		overlayMuxi.blend = ADD;
		uiGroup.add(overlayMuxi);
		...
		var barImage:String;
		if(curSong.endsWith('-Classic'))
		{
			barImage = 'healthBar-classic';
		}
		else if(SONG.player1.contains('past') && SONG.player2.contains('past') && SONG.gfVersion.contains('past') && curSong != 'Full-Clip')
		{
			barImage = 'healthBar-original';
		}
		else
		{
			barImage = 'healthBar';
		...
		healthBar = new Bar(0, FlxG.height * (!ClientPrefs.data.downScroll ? 0.89 : 0.11), barImage, function() return health, 0, 2);
		...
		{
			vcrShader = new VCRShader();
			add(vcrShader);
	
			chromShader = new ChromaticAbberation();
			add(chromShader);

			vcrFilter = new ShaderFilter(vcrShader.shader);
			chromFilter = new ShaderFilter(chromShader.shader);

			camGame.filters = [vcrFilter, chromFilter];
		}
	}
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer)
			{
				...
				var introImagesArray:Array<String> = switch(stageUI) {
					...
					default: ['UI/${stageUI}UI/ready', 'UI/${stageUI}UI/set', 'UI/${stageUI}UI/go'];
				}
				introAssets.set(stageUI, introImagesArray);
	...
	override public function update(elapsed:Float)
	{
		if(ClientPrefs.data.shaders)
		{
			vcrShader.iTime += elapsed * playbackRate;
			chromShader.iTime = 0.003 * playbackRate;
		}
		...
		if(!healthBarVisible)
		{
			...
			iconP2.alpha = 0.00001;
			healthBar.alpha = 0.00001;
			scoreTxt.alpha = 0.00001;
		}
		else
		{
		...
		{
			health = 0;	// RESET = Quick Game Over Screen 
			trace("RESET = True");
		}
		doDeathCheck();
	...
	function openChartEditor()
	{
		...
		#if desktop
		DiscordClient.changePresence("In The Chart Editor", "Charting duh.", null, true);
	...
	function openCharacterEditor()
	{
		...
		MusicBeatState.switchState(new states.editors.CharacterEditorState(SONG.player2));
	}
		...
		switch(eventName) {
			...
			case 'Set Cam Zoom':
				if(ClientPrefs.data.camZooms) {
					...
					var theZoom:Float = Std.parseFloat(value1);
					if(Math.isNaN(theZoom)) theZoom = defaultCamZoom; //fix

					var duration:Float = Std.parseFloat(value2);
					if(Math.isNaN(duration)) duration = 1; //fix

					camTween = FlxTween.tween(camGame, {zoom: theZoom}, duration, {
						ease: FlxEase.sineInOut,
						onComplete: function (tween:FlxTween) 
						{
							defaultCamZoom = camGame.zoom;
						}
					}); //the thingy, literally a lua to haxe port lmao.
				}
			...
			case 'Add Camera Flash':
				if(ClientPrefs.data.flashing) {	
					...
					if(Math.isNaN(duration)) duration = 1; //fix
					camOther.flash(FlxColor.WHITE, duration, null, true);
				}		

			case 'Add Black Camera Flash':
				if(ClientPrefs.data.flashing) {					
					var duration:Float = Std.parseFloat(value1);
					if(Math.isNaN(duration)) duration = 1; //fix
					camOther.flash(FlxColor.BLACK, duration, null, true);
				}	

			case 'Add Black Fade Out':
				if(ClientPrefs.data.flashing) {				
					var duration:Float = Std.parseFloat(value1);
					if(Math.isNaN(duration)) duration = 1.1; //fix
					camGame.fade(FlxColor.BLACK, duration, false);
					camHUD.fade(FlxColor.BLACK, duration, false);
				}
			...
			case 'Toggle Trail':
				...
				if(flValue1 == 1)
				{
					trailDad = new FlxTrail(dad, null, 5, 6, 0.6, 0.15);
					trailDad.color = FlxColor.RED;
					if(dad.noAntialiasing)
					{
						trailDad.antialiasing = false;
					}
					addBehindDad(trailDad);
				}
				else if (flValue1 == 0 && trailDad != null)
				{
					remove(trailDad);
				}
				...
				if(flValue2 == 1)
				{
					trailBF = new FlxTrail(boyfriend, null, 5, 6, 0.6, 0.15);
					trailBF.color = FlxColor.BLUE;
					if(boyfriend.noAntialiasing)
					{
						trailBF.antialiasing = false;
					}
					addBehindBF(trailBF);
				}
				else if (flValue2 == 0 && trailBF != null)
				{
					remove(trailBF);
				}
	...
	function setParticles() {
		var currentBeat = (Conductor.songPosition / 500);

		var particle:FlxSprite = new FlxSprite(randomBetween(500, 2000), 1600).loadGraphic(Paths.image('UI/particles'));
		particle.scale.set(0.5, 0.5);
		add(particle);

		FlxTween.tween(particle, {x: -900 * Math.sin((currentBeat + 1 * 0.1) * Math.PI)}, 6);
		FlxTween.tween(particle, {y: -900 * Math.tan((currentBeat + 1 * 0.1) * Math.PI)}, 6);
	...
	public function endSong()
	{
		...
		if(ret != FunkinLua.Function_Stop && !transitioning)
		{
			...
			if (isStoryMode)
			{
				...
				else if (isStoryMode && storyPlaylist.length >= 0)
				{
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		if(daNote.noteType == 'Bullet Note') {
			if(boyfriend.animOffsets.exists('hurt')) {
				boyfriend.playAnim('hurt', true); //if you miss the note, you get hurt and loses health
				boyfriend.specialAnim = true;

				FlxG.sound.play(Paths.sound('shoot')); //we play the gun shot sound taked from fnf hd
			}
			else if (curSong.toLowerCase() == 'schizophrenzy' && boyfriend.animOffsets.exists('firstDead')) {
				boyfriend.playAnim('firstDead', true); //if you miss the note, you get hurt and loses health
				health -= 0.15; //you loses health tho :o
			}

			if(dad.animOffsets.exists('shoot')) {
				dad.playAnim('shoot', true); //the opponent also plays the shoot animation
				dad.specialAnim = true;
			}
			else if (curSong.toLowerCase() == 'schizophrenzy') {
				dad.playAnim('singRIGHT', true); //if you miss the note at schizophrenzy, the opponent "hurts" you instead lol
				dad.specialAnim = true;
			}
		}
	...
	public function goodNoteHit(note:Note):Void
	{
		...
		if(note.noteType == 'Bullet Note') {
			if(boyfriend.animOffsets.exists('dodge')) {
				boyfriend.playAnim('dodge', true); //if you press the note, you dodge
				boyfriend.specialAnim = true;
			}
			else if(curSong.toLowerCase() == 'schizophrenzy' && (boyfriend.animOffsets.exists('shoot'))) {
				boyfriend.playAnim('shoot', true); //if you press the note on schizophrenzy, you shoot at the opponent
				boyfriend.specialAnim = true;
				health += 0.15; //also you get more health
			}
			...
			if(dad.animOffsets.exists('shoot')) {
				...
				dad.playAnim('singLEFT', true); //the opponent instead dodges if schizophrenzy is the curSong
				dad.specialAnim = true;
			}
		}

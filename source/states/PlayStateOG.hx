...
class PlayStateOG extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'philly': //Week 1 -- Fight-or-Flight
				if(!ClientPrefs.lowQuality) {
				...
				picoText.cameras = [camTexts];
				add(picoText);

			case 'philly-2': //Week 1 -- Showdown
				if(!ClientPrefs.lowQuality) {
					bg = new BGSprite('philly2/sky', -100, 0, 0.1, 0.1);
					add(bg);
				}

				city = new BGSprite('philly2/city', -10, 0, 0.3, 0.3);
				city.setGraphicSize(Std.int(city.width * 0.85));
				city.updateHitbox();
				add(city);

				phillyLightsColors = [0xFF31A2FD, 0xFF31FD8C, 0xFFFB33F5, 0xFFFD4531, 0xFFFBA633];
				phillyWindow = new BGSprite('philly2/window', city.x, city.y, 0.3, 0.3);
				phillyWindow.setGraphicSize(Std.int(phillyWindow.width * 0.85));
				phillyWindow.updateHitbox();
				add(phillyWindow);
				phillyWindow.alpha = 0;
				...
				mathxMiedoPng = new BGSprite('philly2/el_mathx', 1260, 340, 0.95, 0.95, ['mathx idle']);
				mathxMiedoPng.setGraphicSize(Std.int(mathxMiedoPng.width * 0.8));
				add(mathxMiedoPng);

				yupodElao = new BGSprite('philly2/yupodCongelao', 1085, 300, 0.95, 0.95, ['webadas']);
				yupodElao.setGraphicSize(Std.int(yupodElao.width * 0.85));
				add(yupodElao);
			...
			case 'philly-3': //Week 1 -- Fallen-Soldier
				if(!ClientPrefs.lowQuality) {
					bg = new BGSprite('philly3/sky', -100, 0, 0.1, 0.1);
					bg.alpha = 0.0001;
					add(bg);
				}

				overlay = new BGSprite('philly3/overlay', 0, 0, 0.9, 0.9);
				...
				city = new BGSprite('philly3/city', -10, 0, 0.3, 0.3);
				city.setGraphicSize(Std.int(city.width * 0.85));
				city.updateHitbox();
				city.alpha = 0.0001;
				add(city);
				...
				phillyStreet.alpha = 0.0001;
				add(phillyStreet);

				mathxMiedoPng = new BGSprite('philly3/el_mathx', 1260, 340, 0.95, 0.95, ['mathx idle']);
				mathxMiedoPng.setGraphicSize(Std.int(mathxMiedoPng.width * 0.8));
				mathxMiedoPng.alpha = 0.0001;
				add(mathxMiedoPng);

				yupodElao = new BGSprite('philly3/yupodCongelao', 1085, 300, 0.95, 0.95, ['webadas']);
				yupodElao.setGraphicSize(Std.int(yupodElao.width * 0.8));
				yupodElao.alpha = 0.0001;
				add(yupodElao);
				...
				pogaCorrupted.alpha = 0.0001;
				pogaCorrupted.setGraphicSize(Std.int(pogaCorrupted.width * 1.5));

			case 'limo': //Week 4 -- ???
				var skyBG:BGSprite = new BGSprite('street/sky', -300, -500, 0.1, 0.1);
				skyBG.setGraphicSize(Std.int(skyBG.width * 1.2));
				skyBG.updateHitbox();
				add(skyBG);

				var city:BGSprite = new BGSprite('street/city', -500, -100, 0.4, 0.4);
				add(city);

				var sign:BGSprite = new BGSprite('street/sign', -100, -100, 0.6, 0.6);
				add(sign);

				var thePole:BGSprite = new BGSprite('street/pole', 0, -50, 0.8, 0.8);
				add(thePole);

				var street:BGSprite = new BGSprite('street/street', -450, 800, 0.95, 0.95);
				street.setGraphicSize(Std.int(street.width * 1.2));
				street.updateHitbox();
				add(street);

				var limo:BGSprite = new BGSprite('street/limoStand', -120, -285, 1, 1);
				add(limo);
		...
		if(curStage == 'philly-2'){
			add(pogaCorrupted);
		}

		if(curStage == 'philly-3'){
			add(pogaCorrupted);
			add(white);
		}
	...
	public function startCountdown():Void
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer)
			{
				...
				// head bopping for bg characters on Philly
				if(curStage == 'philly') {
				if(!ClientPrefs.lowQuality)
					{
						skidLmao.dance(true);
				...
				if(curStage == 'philly-2' || curStage == 'philly-3') {
					if(!ClientPrefs.lowQuality)
					{
						skidLmao.dance(true);
	...
	override function beatHit()
	{
		...
		if (curBeat == 59 && curSong == 'Fight-Or-Flight')
			{
				FlxTween.tween(picoText, {alpha: 1}, 1.2);
			}

		if (curBeat == 63 && curSong == 'Fight-Or-Flight')
			{
				FlxTween.tween(picoText, {alpha: 0}, 0.35);
			}

		if (curBeat == 480 && curSong == 'Showdown')
			{
				camHUD.visible = false;
				camGame.visible = false;
			}

		if (curBeat == 16 && curSong == 'Fallen-Soldier')
			{
				FlxTween.tween(dad, {alpha: 1}, 1);
			}
			
		if (curBeat == 32 && curSong == 'Fallen-Soldier')
			{
				FlxTween.tween(boyfriend, {alpha: 1}, 1);
			}

		if (curBeat == 48 && curSong == 'Fallen-Soldier')
			{
				FlxTween.tween(gf, {alpha: 1}, 1);
			}

		if (curBeat == 76 && curSong == 'Fallen-Soldier')
			{
				FlxTween.tween(white, {alpha: 1}, 1.5);
			}	
			
		if (curBeat == 80 && curSong == 'Fallen-Soldier')
			{
		...
		if (curBeat == 192 && curSong == 'Fallen-Soldier')
			{
				FlxTween.tween(pogaCorrupted, {alpha: 0}, 1); 
			}
		if (curBeat == 256 && curSong == 'Fallen-Soldier')
			{

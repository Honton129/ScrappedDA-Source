...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		'No Animation',
		'Dad Duality Sing',
		'Duality Parents Duet',
		'Pico Dead Sing',
		'Pico Dead Duet',
		'GF Tempest Sing',
		'Tempest Bf Gf Duet',
		'Tempest Gf Dad Duet',
		'Tempest Opponents Trio',
		'Spooky Tempest Sing',
		'Mom Tempest Sing',
		'Tempest Pico Spooky Duet',
		'Tempest Spooky Mom Duet',
		'Tempest Pico Mom Duet',
		'Tempest Player Trio'
	];
	...
	[
		...
		['Blank Event', "Very Similar to the empty event\nbut this one lets you call stuff on lua\nusing the value 1 and 2 recalls!"],
		['Hey!', "Plays the \"Hey!\" animation from Bopeebo,\nValue 1: BF = Only Boyfriend, GF = Only Girlfriend,\nSomething else = Both.\nValue 2: Custom animation duration,\nleave it blank for 0.6s"],
		['Set GF Speed', "Sets GF head bopping speed,\nValue 1: 1 = Normal speed,\n2 = 1/2 speed, 4 = 1/4 speed etc.\nUsed on Fresh during the beatbox parts.\n\nWarning: Value must be integer!"],
		['Philly Glow', "Exclusive to Pico Week\nValue 1: 0/1/2 = OFF/ON/Reset Gradient\n \nNo, i won't add it to other weeks."],
		['BG Freaks Expression', "Should be used only in \"school\" Stages!"],
		['Trigger BG Ghouls', "Should be used only in \"schoolEvil\" Stage!"],
		['Toggle Senpai BG', "Should be used only in \"school\" Stages!\nValue 1: 1 for change the current stage\n0 for make the stage go back to its original state"],
		['Dadbattle Spotlight', "Used in Dad Battle,\nValue 1: 0/1 = ON/OFF,\n2 = Target Dad\n3 = Target BF"],
		['Add Camera Zoom', "Used on MILF on that one \"hard\" part\nValue 1: Camera zoom add (Default: 0.015)\nValue 2: UI zoom add (Default: 0.03)\nLeave the values blank if you want to use Default."],
		['Set Cam Zoom', "Sets Camera Zoom\nValue 1: Camera zoom value\nValue 2: Time (Default: 1)\nUse 0.001 for an instant zoom!"],
		['Add Camera Flash', "Generates a White Screen Flash"],
		['Add Black Camera Flash', "Generates a Black Screen Flash"],
		['Add Black Fade Out', "Generates a Black Screen Fade"],
	...
	**/
	...
	override function create()
	{
		...
		#if DISCORD_ALLOWED
		DiscordClient.changePresence("in The Chart Editor", 'Charting ' + StringTools.replace(_song.song, '-', ' ') + ' duh.');
		#end
		...
		var tipText:FlxText = new FlxText(1100, 700, 300, "Press F1 for Help", 16);
		tipText.scrollFactor.set();
		add(tipText);

		//lil buddies
		lilStage = new FlxSprite(18, 432).loadGraphic(Paths.image("editors/chartEditor/lilStage"));
		lilStage.antialiasing = false;
		lilStage.scrollFactor.set();
		add(lilStage);

		lilBf = new FlxSprite(18, 432).loadGraphic(Paths.image("editors/chartEditor/lilBf"), true, 300, 256);
		...
		lilOpp = new FlxSprite(18, 432).loadGraphic(Paths.image("editors/chartEditor/lilOpp"), true, 300, 256);
		...
		var eventIcon:FlxSprite = new FlxSprite(-GRID_SIZE - 5, -90).loadGraphic(Paths.image('editors/chartEditor/eventArrow'));
		...
		quant = new AttachedSprite('editors/chartEditor/chart_quant','chart_quant');
		quant.animation.addByPrefix('q','chart_quant',0,false);
		quant.antialiasing = ClientPrefs.data.antialiasing;
	...
	var check_lilbuds:FlxUICheckBox = null; //lil buddies
	var check_moveLilbuds:FlxUICheckBox = null; //lil buddies
	...
	function addChartingUI() {
		...
		//lil buddies
		check_lilbuds = new FlxUICheckBox(mouseScrollingQuant.x + 120, mouseScrollingQuant.y, null, null, "Lil' Buddies", 100);
		check_lilbuds.checked = true;
		check_lilbuds.callback = function()
		{
			lilBf.visible = check_lilbuds.checked;
			lilOpp.visible = check_lilbuds.checked;
			lilStage.visible = check_lilbuds.checked;

			FlxG.sound.play(Paths.sound('scrollMenu'));
		};

		check_moveLilbuds = new FlxUICheckBox(mouseScrollingQuant.x + 120, mouseScrollingQuant.y + 20, null, null, "Flip Lil' Buddies", 100);
		check_moveLilbuds.checked = false;
		check_moveLilbuds.callback = function()
		{
			moveRight = check_moveLilbuds.checked;
		...
		tab_group_chart.add(check_lilbuds); //lil buddies
		tab_group_chart.add(check_moveLilbuds); //lil buddies
	...
	override function update(elapsed:Float)
	{
		...
		FlxG.mouse.visible = true; //cuz reasons, trust me.
		...
		if (!blockInput)
		{
			...
			if(FlxG.keys.justPressed.F1 || (helpBg.visible && FlxG.keys.justPressed.ENTER) || (helpBg.visible && FlxG.keys.justPressed.ESCAPE))
			{
		...
		var playedSoundOppo:Array<Bool> = [false, false, false, false];
		curRenderedNotes.forEachAlive(function(note:Note) {
			note.alpha = 1;
			if(curSelectedNote != null) {
			...
			if(note.strumTime <= Conductor.songPosition) {
				...
				if(note.strumTime > lastConductorPos && FlxG.sound.music.playing && note.noteData > -1) {
					...
					if(!playedSound[data]) {
						...
						else if((!playSoundBf.checked && note.mustPress)){
							lilBf.animation.play("" + Std.string(data), true);
						}	
					}	
					
					if(!playedSoundOppo[data]) {
					    if((playSoundDad.checked && !note.mustPress)){
							var soundToPlay = 'hitsound2';
							
							lilOpp.animation.play("" + Std.string(data), true);
							
							FlxG.sound.play(Paths.sound(soundToPlay)).pan = note.noteData < 4? -0.3 : 0.3; //would be coolio
							playedSoundOppo[data] = true;
						}
						else if((!playSoundDad.checked && !note.mustPress)){
							lilOpp.animation.play("" + Std.string(data), true);
						}	
					}
	...
	{
		var str:String = "CHARTING EDITOR
		...
		\nShift + Enter - Play your chart on the current section.";
		...
		for (i in 0...arr.length)
		{
			...
			var helpText:FlxText = new FlxText(0, 0, 750, arr[i], 16);
	...
	function reloadGridLayer() {
		...
		//
		gridBGOverlay = FlxGridOverlay.create(GRID_SIZE * 9, GRID_SIZE * 4, GRID_SIZE * 12, GRID_SIZE * 16 * gridBG2Length, true, 0xFFFFFFFF, 0xFFB5A5CE);
		gridBGOverlay.antialiasing = false;
		gridBGOverlay.blend = "multiply";
		gridLayer.add(gridBGOverlay);
		//
	...
	{
		var healthIconP1:String = loadHealthIconFromCharacter('bf');
		var healthIconP2:String = loadHealthIconFromCharacter('dad');
	...
	function setupNoteData(i:Array<Dynamic>, isNextSection:Bool):Note
	{
		...
		if(daSus != null) { //Common note
			...
			note.loadGraphic(Paths.image('editors/chartEditor/eventArrow'));

...
class WeekEditorState extends MusicBeatState
{
	...
	override function create() {
		...
		weekThing = new MenuItem(10, bgSprite.y + 396, weekFileName);
		...
		lock = new FlxSprite(weekThing.width + 10 + weekThing.x);
		lock.frames = ui_tex;
	...
	function addWeekUI() {
		...
		backgroundInputText = new FlxUIInputText(10, songsInputText.y + 60, 120, '', 8);
	...
	function addOtherUI() {
		...
		tab_group.add(new FlxText(difficultiesInputText.x, difficultiesInputText.y + 20, 0, 'Default difficulties are "Canon, Secure"\nwithout quotes.'));
	...
	function reloadWeekThing() {
		...
		var isMissing:Bool = true;
		if(assetName != null && assetName.length > 0) {
			if( #if MODS_ALLOWED FileSystem.exists(Paths.modsImages('story;enu/' + assetName)) || #end
		...
		#if DISCORD_ALLOWED
		DiscordClient.changePresence("in the Week Editor", "Editting: " + weekFileName);
		#end
	}
	...
	override function update(elapsed:Float)
	{
		...
		if(!blockInput) {
			FlxG.sound.muteKeys = WarningState.muteKeys;
			FlxG.sound.volumeDownKeys = WarningState.volumeDownKeys;
			FlxG.sound.volumeUpKeys = WarningState.volumeUpKeys;
